...
async function createPost() {
  ...
  const toxicRes = await fetch("http://localhost:5000/check-toxicity", {
    method: "POST",
    headers: {
  ...
  if (!toxicData.success) {
    alert(toxicData.message); // ❌ BLOCK POST
    return;
  }
...
function renderPosts(posts) {
  ...
  posts.forEach(p => {
    ...
    <div class="card mb-3 p-3">
      ...
      <button onclick="editPost(${p.id}, \`${p.title}\`, \`${p.content}\`)" 
        class="btn btn-sm btn-warning mt-2">Edit</button>
